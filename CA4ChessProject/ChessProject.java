...
public class ChessProject extends JFrame implements MouseListener, MouseMotionListener {
   ...
   * checks the grid below:
   * 
   * 
   * _|_____________|_________|_____________|_ | | | | |(x-75, y-75) |(x,
   * y-75)|(x+75, y-75) | _|_____________|_________|_____________|_ | | | |
   * |(x-75, y) | (x, y) |(x+75, y) | _|_____________|_________|_____________|_ |
   ...
   * direction as long as there is not another king in an adajacent square to were
  ...
  private Stack getKingSquares(int x, int y, String piece) {
    ...
    /*
     * If we consider the grid above, we can create three different columes to
   ...
   * below grid, the Bishop can move in a diagonal moement. There are essentially
  ...
  /*
   * Method to find all the White Pieces. This code is coming from Code Snippet 2:
   * Method to find the positions of all the White pieces on the board
   */
    ...
    for (int i = 0; i < 600; i += 75) {
      for (int j = 0; j < 600; j += 75) {
        ...
        if (tmp instanceof JLabel) {
          ...
          if (pieceName.contains("White")) {// Checking if pieceName is = White
  ...
  public void mouseReleased(MouseEvent e) {
    ...
    if (possible) {
      ...
      if (pieceName.contains("Bishop")) {
        /*
         * The Bishop is a simple piece and has a very long range so usually is used to
         * attack from far away. It moves along the diagonal as long as nothing is in
         * the way to its destination. Like all other Chess pieces it can take its
         * opponent piece but cannot take its own piece.
         */
        ...
        if (((landingX < 0) || (landingX > 7)) || ((landingY < 0) || (landingY > 7))) {
          ...
          validMove = true;
          /*
           * The condition to check for a validMove simple asks is the x movement is the
           * same as the y movement, if so we are moving along a diagonal. We then need to
           * identify which diagonal and make sure there is nothing in the way.
           */
          if (xMovement == yMovement) {
            ...
            if (inTheWay) {
              ...
              /*
               * This is cool there is nothing in the way and we just need to make sure that
               * we are not able to take our own piece.
               */
        ...
        /*
         * The Rook moves along the x direction or the y direction any number of squares
         * as long as there is nothing in its way. It can take an opponents piece and as
         * you know it cannot take its own piece.
         */
        Boolean intheway = false;
        if (((landingX < 0) || (landingX > 7)) || ((landingY < 0) || (landingY > 7))) {
          validMove = false;
          ...
          /*
           * The following condition simply asks if we have either a movement in the x
          ...
          if (((xMovement != 0) && (yMovement == 0)) || ((xMovement == 0) && (yMovement != 0))) {
            if (xMovement != 0) { // we have a movement in the x direction
              if (startX - landingX > 0) {
                /*
                 * We need to check all the squares up to the landing square to make sure
            ...
            if (intheway) {
              /*
               * if we have found that there is something in the way to the square we are
               * trying to move to we simply set the validMove boolean to false...
               */
              validMove = false;
            } else {
              /*
               * okay so we have established that there is nothing in the way and we want to
               * make try and move the piece. The other condiion that we need to check is to
               * see if there is a piece at the landing square that we want to move to.
               */
              if (piecePresent(e.getX(), (e.getY()))) {
                if (pieceName.contains("White")) {
                  if (checkWhiteOponent(e.getX(), e.getY())) {
                    validMove = true;
                    /*
                     * Every time we make a take a piece we need to check if the piece that has been
                     * captured is a King.
                     */
        ...
        /*
         * We know that the Knight can move in an L direction. It means that if the
         * xMovement == 1 then the yMovement must equal 2 and also the other way
         * around... We need to check the square that we are moving to and make sure
         * that if there is a piece present that its not our own piece...
         */
        ...
        if ((xMovement == 0) && (yMovement == 0)) {
          ...
          if (((landingX < 0) || (landingX > 7)) || ((landingY < 0) || (landingY > 7))) {
            ...
            if ((xMovement > 1) || (yMovement > 1)) {
              ...
              /*
               * The Kind can only move one square at a time but also needs to make sure that
               * there is at least one square between both Kings once the move has concluded.
               */
              if ((getPieceName((e.getX() + 75), e.getY()).contains("King"))
                ...
                /*
                 * We need to make sure that if there is a piece on the potential landing square
                 * that the King cannot takes its own piece but can take an opponents piece.
                 */
                if (piecePresent(e.getX(), e.getY())) {
                  if (pieceName.contains("White")) {
                    if (checkWhiteOponent(e.getX(), e.getY())) {
        ...
        if (((landingX < 0) || (landingX > 7)) || ((landingY < 0) || (landingY > 7))) {
          ...
          /*
           * The Queen can either move like the Rook or a Bishop...You already have this
           * code working!
           * 
          ...
          if (xMovement == yMovement) {
            ...
            if (inTheWay) {
              ...
              /*
               * If there is nothing in the way when moving the Queen like a Bishop, we have
               * two cases: - there is a piece on the square we are trying to move to and we
               * need to make sure that its an enemy piece - or there is nothing in the way
            ...
            /*
             * This is the condition that handles the Queen movements that are moving like a
             * Rook
             */
            if (xMovement != 0) {
            ...
            if (inTheWay) {
              ...
              /*
               * This is cool and just like before there is now nothing in the way and the
               * Queen should be able to make the movement. We need to check if there is
               * something on the square that we are moving to and if so make sure its an
               * enemy piece and if there is nothing on the square that we are moving to ...
               * make the move.
               */
              if (piecePresent(e.getX(), (e.getY()))) {
                if (pieceName.contains("White")) {
        ...
        if (startY == 6) { // This is were the pawn is making its first move....
          /*
           * if the pawn is makeing ites first movement.... the pawn can either move one
           * square or two squares...in the Y direction as long as we are moving up the
           * board!!! and also there is no movement in the x direction
           */
          ...
          if (((yMovement == 1)) && (startY > landingY) && (xMovement == 0)) {
            if (!piecePresent(e.getX(), e.getY())) {
              validMove = true;
              /*
               * We need a variable to inform us that the Black Pawn has reached the top of
               * the board and can now trun into a Black Queen
               */
              if (landingY == 0) {
                progression = true;
              }
        ...
        if (startY == 1) {
          if (((xMovement == 0)) && ((yMovement == 1) || ((yMovement) == 2))) {
            if (yMovement == 2) {
              ...
              && (startY < landingY)) {
            /*
             * This is the condition were we are checking for a move to take a piece when
             * the white pawn is making its first move. We check the following: - that there
             * is a piece present, - we are moving in a diagonal, - the movement is only one
             * square, - we are moving down the board
             */
            if (checkWhiteOponent(e.getX(), e.getY())) {
              validMove = true;
              if (startY == 6) {
    ...
    if (!validMove) {
      ...
      panels.add(pieces);
    } else { // this is the condition where we have a valid move and need to visually show
             // it.
      if (white2Move) {
        white2Move = false;
      } else {
